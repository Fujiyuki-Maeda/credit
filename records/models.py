...
# ▼▼▼ 変更点: ImportBatchをRecordの前に定義 ▼▼▼
class ImportBatch(models.Model):
    """CSVインポート処理を記録するモデル"""
    timestamp = models.DateTimeField("インポート日時", auto_now_add=True)
    # file_name = models.CharField("ファイル名", max_length=255, null=True) # 将来的にファイル名も記録できる

    def __str__(self):
        return f"インポート処理 @ {self.timestamp.strftime('%Y-%m-%d %H:%M')}"
...
class Record(models.Model):
    ...
    installments = models.IntegerField("分割回数", null=True, blank=True)
    receipt_amount = models.DecimalField("レシート金額", max_digits=10, decimal_places=0, null=True, blank=True)

    # on_delete=models.CASCADE は、バッチが削除されたら関連するレコードも一緒に削除するという設定
    batch = models.ForeignKey(ImportBatch, on_delete=models.CASCADE, null=True, related_name='records')
